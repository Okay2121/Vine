...
def get_loss_probability():
    """Get the probability of loss days."""
    loss_prob = get_system_setting('loss_probability', config.SIMULATED_LOSS_PROBABILITY)
    return float(loss_prob)

def get_support_username():
    """Get the support username."""
    username = get_system_setting('support_username', '@admin')
    return username