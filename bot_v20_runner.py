...
def time_selection_handler(update, chat_id):
    ...
    try:
        ...
        # Check if it's a preset time selection
        if callback_data.startswith('time_'):
            time_option = callback_data.replace('time_', '')
            current_time = datetime.utcnow()
            
            # Calculate the selected time
            if time_option == '5min':
                selected_time = current_time - timedelta(minutes=5)
            elif time_option == '15min':
                selected_time = current_time - timedelta(minutes=15)
            elif time_option == '1hr':
                selected_time = current_time - timedelta(hours=1)
            elif time_option == '3hr':
                selected_time = current_time - timedelta(hours=3)
            elif time_option == '6hr':
                selected_time = current_time - timedelta(hours=6)
            elif time_option == '12hr':
                selected_time = current_time - timedelta(hours=12)
            elif time_option == 'auto':
                selected_time = current_time
            elif time_option == 'custom':
                # Handle custom time input
                custom_time_msg = (
                    "🕐 *Custom Time Entry*\n\n"
                    "Enter the exact time when this trade was executed:\n\n"
            ...
            # Default to current time if no valid selection
            process_trade_broadcast_with_timestamp(chat_id, datetime.utcnow())
            
    except Exception as e:
        import logging
        logging.error(f"Error in time_selection_handler: {str(e)}")
        bot.send_message(chat_id, "❌ Error processing time selection.")
...
def process_trade_broadcast_with_timestamp(chat_id, timestamp):
    ...
    try:
        ...
        if not hasattr(bot, '_pending_trade_data'):
            bot.send_message(chat_id, "❌ No pending trade data found.")
            return
        ...
        import logging
        logging.error(f"Error in process_trade_broadcast_with_timestamp: {str(e)}")
        bot.send_message(chat_id, "❌ Error processing trade broadcast.")
    ...
    try:
        ...
        try:
            ...
            error_msg = (
                ...
                "📅 *Format:* YYYY-MM-DD HH:MM\n"
                "🕐 *Example:* 2025-07-05 14:30\n\n"
                "Try again or use /cancel to go back."
            )
        ...
        import logging
        logging.error(f"Error in custom_time_input_handler: {str(e)}")
